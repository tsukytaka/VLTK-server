...
----------------------------------------------------------------------------------------------------
-- Mo nhanh Vo Lam Lien Dau - Don dau tu do.
-- Duoc goi tu Lenh bai tinh nang, muc 22.
function StartLienDauNhanh()
	local _, _, n_sid = wlls_calc_phase()
	if not n_sid or not WLLS_SEASON_TB[n_sid] then
		OutputMsg("StartLienDauNhanh ERROR: khong tim thay mua lien dau hien tai")
		return
	end

	local n_mid = GetGblInt(RLGLB_WLLS_MATCHID)
	if GetGblInt(RLGLB_WLLS_PHASE) ~= 4 or not n_mid or n_mid <= 0 then
		local n_hour = tonumber(date("%H"))
		local n_min = tonumber(date("%M"))
		local n_loop = mod(n_hour * 4 + floor(n_min / 15), 99) + 1
		n_mid = tonumber(date("%y%m%d")) * 100 + n_loop
	end

	SetGblInt(RLGLB_WLLS_PHASE, 4)
	wlls_set_mid(n_sid, n_mid)
	wlls_set_phase(4, n_mid, n_sid)
	OutputMsg("StartLienDauNhanh OK: Phase=4 MatchID="..n_mid.." SeasonID="..n_sid)
end
