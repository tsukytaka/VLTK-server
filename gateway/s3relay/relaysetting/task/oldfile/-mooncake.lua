...
function TaskShedule()
	TaskName("Trung thu ®æi lµm tİn lÖnh bµi");	
	...
	TaskTime(h, m);
	OutputMsg(format("Trung thu ®æi lµm tİn lÖnh bµi, khëi ®éng thêi gian lµ %d:%d...", h, m));
...
function CloseLastExchange()
    ...
    if (lastcity ~= nil and lastcity ~= 0) then
    	...
    	OutputMsg(format("Trung thu ®æi lµm tİn lÖnh bµi % ®ãng...", cityname))
        -- ´¥·¢GameServerÉÏµÄ½Å±¾
...
function TaskContent()
    ...
    if (hour >= START_HOUR_TIME) then
        ...
        if (typeid == nil) then
        	OutputMsg("Trung thu ®æi lµm tİn lÖnh bµi sai...")
        else
    	    -- ´¥·¢GameServerÉÏµÄ½Å±¾
            CloseLastExchange()
            ...
            GlobalExecute(cmd)
        	OutputMsg(format("Trung thu ®æi lµm tİn lÖnh bµi % më míi...", cityname));
