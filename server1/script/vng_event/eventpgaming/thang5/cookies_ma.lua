...
local nDayNow = "20"..nYMD..""
	...
	for i= 1, nteam do
		...
		if GetTask(TSK_33_COOKIES_DATE) == nil or GetTask(TSK_33_COOKIES_DATE) == 0 then
			SetTask(TSK_33_COOKIES_DATE,nDayNow);
			SetTask(TSK_33_COOKIES_COUNT,0);
		end
		
		if (tonumber(GetTask(TSK_33_COOKIES_DATE)) == nDayNow and tonumber(GetTask(TSK_33_COOKIES_COUNT)) >= 1000) then
			PlayerIndex = oldPlayer;
			Say("B¹n ®· qu¸ mÖt råi, mai míi cã thÓ tiÕp tôc lµm b¸nh. Mçi ngµy chØ ®ñ søc lµm tèi ®a 1000 lÇn.",0);
			return 1;
		end
		...
		if (checkcookies2007Limit() ~= 1) then
			PlayerIndex = oldPlayer;
			Say("ThËt ng¹i qu¸, chØ cã ng­êi ch¬i <color=yellow>cÊp trªn 50<color> míi cã thÓ sö dông bao nguyªn liÖu lµm b¸nh, mét trong hai vÞ hiÖp kh¸ch ch­a ®¸p øng ®ñ ®iÒu kiÖn nµy råi.", 0);
			return 1;
		end;
	end
	PlayerIndex = oldPlayer;
	...
	if (nNpcIdx > 0) then
		...
		SetNpcScript(nNpcIdx, "\\script\\vng_event\\eventpgaming\\thang5\\pan.lua");
