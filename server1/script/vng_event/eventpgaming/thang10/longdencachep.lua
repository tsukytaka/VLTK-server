...
-----------------------------------------------------------
nTSK_USE_TIMES_LIMIT	 = 5761
nTSK_USE_COUNT_LIMIT	 = 5760
nUSE_COUNT_LIMIT	 = 5
-------------------------------
...
local n_cur_date = tonumber(GetLocalDate("%Y%m%d%H%M"));
	...
	if (GetTask(nTSK_USE_COUNT_LIMIT) >= nUSE_COUNT_LIMIT) then
		Talk(1,"","Mçi Ngµy ChØ ®­îc sö dông 5 lång ®Ìn c¸ chÐp")
		return 1
	end
...
local nDayNow = "20"..nYMD..""
	...
	DropItem(world, x * 32, y * 32, -1, 6, 1, tab_light[5][1], 1, 0, 0, 0);

		AddSkillState(440, 1, 1, 64800 * 0.5);
