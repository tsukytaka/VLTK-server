...
-----------------------------------------------------------
nTSK_USE_TIMES_LIMIT	 = 5759
nTSK_USE_COUNT_LIMIT	 = 5758
nUSE_COUNT_LIMIT	 = 5
-------------------------------
...
local n_cur_date = tonumber(GetLocalDate("%Y%m%d%H%M"));
	...
	if (GetTask(nTSK_USE_COUNT_LIMIT) >= nUSE_COUNT_LIMIT) then
		Talk(1,"","Mçi Ngµy ChØ ®­îc sö dông 5 lång ®Ìn kÐo qu©n")
		return 1
	end
...
local nDayNow = "20"..nYMD..""
	...
	DropItem(world, x * 32, y * 32, -1, 6, 1, tab_light[6][1], 1, 0, 0, 0);

		AddSkillState(440, 1, 1, 64800 * 0.6);
