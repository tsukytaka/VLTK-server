...
function main()
	...
	local bAdminTransLife = GetTaskTemp(198);
	if (not TB_LEVEL_REMAIN_PROP[n_level] or not TB_LEVEL_REMAIN_PROP[n_level][n_transcount+1]) then
		local szError = format("Thieu du lieu Trung sinh: level=%d, lan=%d", n_level, n_transcount+1);
		Msg2Player(szError);
		WriteLog(format("[TransLifeError] Account:%s Name:%s %s", GetAccount(), GetName(), szError));
		SetTaskTemp(198, 0);
		return 0;
	end
	...
	if n_transcount == 3 then
		...
		--·çÔÆÁîÅÆ- µ÷Õû¶ÔÐ¢ÊÖÖØÉúµÄÊ±¼ä- Modified By DinhHQ - 20110926
		if bAdminTransLife == 1 then
			-- Admin support khong tru phi.
		elseif tbPVLB_Check:IsNewPlayer() == 1 and tbPVLB_Check:CheckTime() == 1 and (n_transcount == 0 or n_transcount == 1)  then
