...
function BossCleanSimCityMark(npcindex)
	if not npcindex or npcindex <= 0 then return end
	if SetNpcParam then
		SetNpcParam(npcindex, 2, 0)
		SetNpcParam(npcindex, 3, 0)
		SetNpcParam(npcindex, 4, 0)
	end
	if SetNpcTitle then SetNpcTitle(npcindex, 0) end
	if SetNpcBang then SetNpcBang(npcindex, "") end
end
